...
double weight=0;
  bool listenToBin() {
    ...
    print(beaconTimeouts);
    sortedsumMap.clear();
    sumMap.forEach((key, value) {
      // Check if the beacon is within the timeout
      if (beaconTimeouts.containsKey(key) &&
          beaconTimeouts[key]!.isAfter(DateTime.now())) {
        return; // Skip beacons in timeout
      }
      ...
      if (widget
        ...
        if (widget.user.key != Building.apibeaconmap[nearestBeacon]!.sId) {
          ...
          // }
              ...
              highestweight >= 3.0) {
            widget.user.onConnection = false;
            beaconTimeouts[nearestBeacon] = DateTime.now().add(Duration(minutes: 1));
            //
            List<int> beaconcoord = [
              Building.apibeaconmap[nearestBeacon]!.coordinateX!,
              Building.apibeaconmap[nearestBeacon]!.coordinateY!
