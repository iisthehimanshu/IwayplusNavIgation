...
class _NavigationState extends State<Navigation> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  UserState user = UserState(
      ...
      key: "",
      theta: 0.0);
  pathState PathState = pathState.withValues(
      -1,
      -1,
      -1,
      -1,
  ...
  void initializePannels() {
    PinLandmarkPannel = pinLandmarkPannel(
      ...
      closePanel: closePinnedLandmarkPannel,
    );
  }
  ...
  Future<void> zoomWhileWait(Map<String, LatLng> allBuildingID,
  ...
  Future<void> setPdrThreshold() async {
    if (SingletonFunctionController.building.patchData[buildingAllApi
        .selectedBuildingID] != null &&
        SingletonFunctionController.building.patchData[buildingAllApi
            .selectedBuildingID]!.patchData!.pdrThreshold != null &&
        SingletonFunctionController.building.patchData[buildingAllApi
            .selectedBuildingID]!.patchData!.pdrThreshold!.isNotEmpty) {
      peakThreshold = double.parse(
          SingletonFunctionController.building.patchData[buildingAllApi
              .selectedBuildingID]!.patchData!.pdrThreshold!);
  ...
  void handleCompassEvents() {
    magnetoData.magnetometerStream.listen((event) {
      if (!mounted)
        return; // Prevent setState if the widget is no longer in the tree
...
// late StreamSubscription<AccelerometerEvent>? pdr;
  ...
  String convertTolng(String msg, String lngcode, String finalvalue) {
    if (msg ==
        "You are on ${tools.numericalToAlphabetical(user.floor)} floor,${user
            .locationName}") {
      if (lngcode == 'en') {
        return msg;
      } else {
        return "आप ${tools.numericalToAlphabetical(user.floor)} मंज़िल, ${user
            .locationName} पर हैं";
      }
    } else if (msg ==
        "You are on ${tools.numericalToAlphabetical(user.floor)} floor,${user
            .locationName} is on your ${LocaleData.properties5[finalvalue]
            ?.getString(context)}") {
      if (lngcode == 'en') {
        return msg;
      } else {
        return "आप ${tools.numericalToAlphabetical(
            user.floor)} मंजिल पर हैं, ${user.locationName} आपके ${LocaleData
            .properties5[finalvalue]?.getString(context)} पर है";
      }
  ...
  Future<Landmarks?> getGlobalCoords(LatLng coordinates) async {
    ...
    // Fetch landmark data
    final landmarkData = await SingletonFunctionController.building
        .landmarkdata;
    ...
    for (var landmark in landmarkData!.landmarks!) {
      if (landmark.buildingID == buildingAllApi.outdoorID &&
        ...
        double dist = tools.calculateAerialDist(
            latLngValue[0], latLngValue[1],
            coordinates.latitude, coordinates.longitude
        );

        if (landmark.properties?.polyId != null &&
            landmark.name?.isNotEmpty == true && dist < minDistance) {
          minDistance = dist;
    ...
    if (closestLandmark == null ||
        Building.waypoint[buildingAllApi.outdoorID] == null)
      return closestLandmark;

    // Find closest waypoint
    Map<double, Nodes> waypoints = {};
    final polylineData = SingletonFunctionController.building.polylinedatamap;
    PathModel model = Building.waypoint[buildingAllApi.outdoorID]!.firstWhere((
        e) => e.floor == 0);
    ...
    polylineData[buildingAllApi.outdoorID]?.polyline?.floors?.forEach((floor) {
      floor.polyArray?.forEach((polyline) {
        if (polyline.polygonType == "Waypoints" && polyline.floor ==
            tools.numericalToAlphabetical(closestLandmark?.floor ?? 0)) {
          for (var node in polyline.nodes ?? []) {
            double dist = tools.calculateAerialDist(
                node.lat!, node.lon!, coordinates.latitude,
    ...
    if (waypoints.isNotEmpty) {
      ...
      for (var waypoint in nearestWaypoints) {
        ...
        List<dynamic>? adjGlobalList = adjGlobal["${waypoint?.lat},${waypoint
            ?.lon}"];
        ...
        ClosestPointResult closestPoint = ClosestPointResult(
            ...
            tools.calculateAerialDist(
                coordinates.latitude, coordinates.longitude, waypoint!.lat!,
                waypoint!.lon!)
        );
        ...
        if (adjList != null && adjGlobalList != null) {
          ...
          closestPoint = shortestPoint(
              coordinates, closestPoint.latLngPoint, closestPoint.intPoint,
              latLngPoints, intPoints);
      ...
      MapEntry<Nodes, ClosestPointResult> shortestProjection = result.entries
          .reduce((a, b) =>
      a.value.projectLength < b.value.projectLength
          ? a
          : b);
      ...
      if (tools.calculateDistance([
      ...
      ], [shortestProjection.key!.coordx!, shortestProjection.key!.coordy!]) <=
          10) {
        closestPoint = ClosestPointResult(
            ...
            IntPoint(shortestProjection.key!.coordx!,
                shortestProjection.key!.coordy!), 0);
      }
  ...
  Future<void> addNearbyLandmarkMarkers(LatLng point, String id, String name,
      bool visible) async {
    final Uint8List iconMarker = await getImagesFromMarker(
        'assets/OptionLandmark.png', 85);
  ...
  Future<void> updateNearbyLandmarkMarkers(MarkerId id) async {
    if (PinedLandmark ==
        SingletonFunctionController.building.listOfNearbyLandmarksToLocalize!
            .where((landmark) => landmark.sId == id.value).first) {
      focusOnPinLandmark(nearbyLandmarks[id]!.position);
    }

    if (!nearbyLandmarks.containsKey(id) || PinedLandmark ==
        SingletonFunctionController.building.listOfNearbyLandmarksToLocalize!
            .where((landmark) => landmark.sId == id.value).first) return;

    final Uint8List selectedMarker = await getImagesFromMarker(
        'assets/LandmarkSelected.png', 85);
    BitmapDescriptor selectedLandmark = BitmapDescriptor.fromBytes(
        selectedMarker);
    ...
    print("Updated NearbyLandmark with ID: ${nearbyLandmarks[id]!.markerId
        .value}  now the list is $nearbyLandmarks");
  }


  ClosestPointResult shortestPoint(LatLng userLatLng, LatLng targetLatLng,
      IntPoint targetintPoint, List<LatLng> latLngPoints,
    ...
    LatLng? projectLatLngIfWithinSegment(LatLng user, LatLng start,
        LatLng end) {
      ...
      print("latlng projected are ${start.latitude + t * bx},${start.longitude +
          t * by}");
    ...
    for (int i = 0; i < latLngPoints.length; i++) {
      ...
      LatLng? projectedLatLng = projectLatLngIfWithinSegment(
          userLatLng, targetLatLng, latLngPoint);
    ...
    double distanceToTargetLatLng = distanceBetweenLatLng(
        userLatLng, targetLatLng);
    if (distanceToTargetLatLng < minDistanceLatLng) {
      return ClosestPointResult(targetLatLng, targetintPoint!,
          tools.calculateAerialDist(
              userLatLng.latitude, userLatLng.longitude, targetLatLng.latitude,
    ...
    return ClosestPointResult(closestLatLngPoint ?? targetLatLng,
        closestIntPoint != null
            ? IntPoint(closestIntPoint.x, closestIntPoint.y)
            : targetintPoint,
        tools.calculateAerialDist(userLatLng.latitude, userLatLng.longitude,
...
// Function to check if calibration is needed
  ...
  void listenToMagnetometeronCalibration() {
    ...
    _magnetometerSubscription =
        magnetometerEvents.listen((MagnetometerEvent event) {
          ...
          // Calculate magnetic field strength
          double magneticFieldStrength = calculateMagneticFieldStrength(
              x, y, z);
  ...
  void paintUser(String? nearestBeacon,
      ...
      {bool speakTTS = true, bool render = true, bool providePinSelection = false}) async {
  ...
  Future<void> _handleBeaconLocalization(String nearestBeacon, bool speakTTS,
    ...
    try {
      ...
      final beaconData = SingletonFunctionController
      ...
      if (providePinSelection) {
        ...
        SingletonFunctionController.building.listOfNearbyLandmarksToLocalize =
            tools.findListOfNearbyLandmark(
                beaconData, landmarkData!.landmarksMap!);
        print("list of nearby landmarks::${SingletonFunctionController.building
            .listOfNearbyLandmarksToLocalize} ${beaconData} ${landmarkData!
            .landmarksMap!}");
        if (SingletonFunctionController.building
            .listOfNearbyLandmarksToLocalize != null) {
          detected = false;
          print("got inside this");

          if (SingletonFunctionController.building
              .listOfNearbyLandmarksToLocalize!.where((test) =>
          test.properties!.isWaypoint == false).length > 1) {
            showListOfNearbyLandmarks(SingletonFunctionController.building
                .listOfNearbyLandmarksToLocalize!);
          } else {
            final userSetLocation = tools.localizefindNearbyLandmark(
                beaconData, landmarkData!.landmarksMap!);
            print("usersetlocation::${userSetLocation}");
            if (userSetLocation != null) {
              initializeUser(userSetLocation, beaconData, speakTTS: speakTTS,
                  render: render);
            } else {
              unableToFindLocation();
            }
        ...
        if (userSetLocation != null) {
          initializeUser(
              userSetLocation, beaconData, speakTTS: speakTTS, render: render);
        } else {
          unableToFindLocation();
        }
      }
  ...
  Future<void> _handlePolygonLocalization(String polyID,
      bool speakTTS,
      bool render) async {
    try {
      final landmarkData = await SingletonFunctionController.building
      ...
      if (userSetLocation != null) {
        initializeUser(
            userSetLocation, null, speakTTS: speakTTS, render: render);
      } else {
        unableToFindLocation();
      }
    } catch (e) {
  ...
  Future<void> _handleGlobalCoordinatesLocalization(bool speakTTS,
      bool render,
      bool providePinSelection) async {
    ...
    if (_kalmanFilter.latitudeEstimate != null &&
        _kalmanFilter.longitudeEstimate != null) {
      ...
      showToast("gps stream is not started");
      unableToFindLocation();
    }
  }
  ...
  Future<void> showListOfNearbyLandmarks(List<Landmarks> landmarks) async {
    SingletonFunctionController.building.floor[landmarks.first.buildingID!] =
        landmarks.first.floor ?? 0;
    await createRooms(
        SingletonFunctionController.building.polylinedatamap[landmarks.first
            .buildingID]!, landmarks.first.floor ?? 0);
    await Future.delayed(Duration(milliseconds: 1000));
    speak(convertTolng(
        "Confirm Your Location Amongst Below Given List", _currentLocale, ''),
        _currentLocale);
    focusOnPinLandmark(LatLng(
        double.parse(landmarks.first.properties!.latitude!),
        double.parse(landmarks.first.properties!.longitude!)));
    for (var landmark in landmarks) {
      addNearbyLandmarkMarkers(LatLng(
          double.parse(landmark.properties!.latitude!),
          double.parse(landmark.properties!.longitude!)), landmark.sId ?? "",
          landmark.name ?? "", !landmark.properties!.isWaypoint);
    }
  }
  ...
  void initializeUser(Landmarks userSetLocation, beacon? localizedBeacon,
    ...
    if (userSetLocation!.doorX != null) {
      print("usercoord fetched ${user.coordX},${user
          .coordY}       ${userSetLocation!.doorX!} ${userSetLocation!
          .doorY!}");
    ...
    UserState.cols =
    SingletonFunctionController.building.floorDimenssion[userSetLocation
        .buildingID]![userSetLocation.floor]![0];
    UserState.rows =
    SingletonFunctionController.building.floorDimenssion[userSetLocation
        .buildingID]![userSetLocation.floor]![1];
    ...
    setState(() {
      ...
      if (widget.directLandID.length < 2 && speakTTS) {
        SingletonFunctionController.building.floor[userSetLocation
            .buildingID!] = userSetLocation.floor!;
    ...
    if (getallnearbylandmark.length > 2) {
      distBetweenLandmarks = tools.calculateDistance([user.coordX, user.coordY],
          [
    ...
    if (userSetLocation == null) {
      ...
      if (speakTTS) {
        if (finalvalue == null) {
          ...
          if (getallnearbylandmark.length > 2 && distBetweenLandmarks <= 20 &&
              finalvalue2 != null) {
            speak(
                "You are on ${tools.numericalToAlphabetical(
                    user.floor)} floor,${user
                    .locationName} is on your ${LocaleData
                    .properties5[finalvalue]?.getString(
                    context)} and ${getallnearbylandmark[1]
                    .name} is on your ${LocaleData.properties5[finalvalue2]
                    ?.getString(context)}",
                _currentLocale);
          } else {
            speak(
    ...
    if (speakTTS) {
      ...
      if (SingletonFunctionController.apibeaconmap[lastBeaconValue] != null) {
        List<double> uvalue = tools.localtoglobal(
            SingletonFunctionController.apibeaconmap[lastBeaconValue]!
                .coordinateX!.toInt(),
            SingletonFunctionController.apibeaconmap[lastBeaconValue]!
                .coordinateY!.toInt(),
            SingletonFunctionController.building.patchData[user.bid]
    ...
    Future.delayed(Duration(milliseconds: 5000)).then((value) {
      if (isCalibrationNeeded(magneticValues) &&
          UserState.lowCompassAccuracy == false) {
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          ...
                          await Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) =>
                                    QRViewExample(frmMainPage: false,)),
                          ).then((value) {
                            setState(() {
                              isLoading = false;
      ...
      if (!rerouting) {
        ...
        SingletonFunctionController.building.landmarkdata!.then((value) async {
          await calculateroute(value.landmarksMap!,
              accessibleby: acc ?? PathState.accessiblePath, autoStart: true)
              .then((value) {
  ...
  Future<void> requestBluetoothConnectPermission() async {
    final PermissionStatus permissionStatus = await Permission.bluetoothScan
        .request();
  ...
  void apiCalls(context) async {
    ...
    print("buildingAllApi.selectedBuildingID ${buildingAllApi
        .selectedBuildingID}");
    ...
    List<String> ids = buildingAllApi
        .getStoredAllBuildingID()
        .keys
        .toList();
    try {
    ...
    print("buildingAllApi.getStoredAllBuildingID() ${buildingAllApi
        .getStoredAllBuildingID()
        .length} ${buildingAllApi
        .getStoredAllBuildingID()
        .keys}");
    Future<void> allBuildingCalls = Future.wait(buildingAllApi
        .getStoredAllBuildingID()
        .entries
        .map((entry) async {
      var key = entry.key;
      ...
      if (key == buildingAllApi.outdoorID &&
        ...
        var globalData = await GlobalAnnotation().fetchGlobalAnnotationData(
            key);
        ...
        if (landmarks != null && landmarks.isNotEmpty) {
          var otherLandmarkdata = await SingletonFunctionController.building
              .landmarkdata;
          otherLandmarkdata?.mergeLandmarks(landmarks);
          SingletonFunctionController.building.landmarkdata =
              Future.value(otherLandmarkdata);
        }
      ...
      try {
        var waypointData = await waypointapi().fetchwaypoint(
            key, outdoor: key == buildingAllApi.outdoorID);
    ...
    if (widget.directLandID.length < 2) {
      print("localize 3 called after ${time
          .difference(DateTime.now())
          .inSeconds} seconds ${SingletonFunctionController
          .SC_LOCALIZED_BEACON}");
      Future.delayed(Duration(seconds: 1));
      print("localize 3 delay ${SingletonFunctionController
          .SC_LOCALIZED_BEACON}");
  ...
  Future<void> localizeUser(
    ...
    if (await FlutterBluePlus.isOn) {
      nearestBeacon =
          findMaxWeightKey(SingletonFunctionController.btadapter.latesILMap);

      Future<void> localizeUser(
          {bool speakTTS = true, String beacon = "", bool providePinSelection = true}) async {
        // for (int i = 0; i < SingletonFunctionController.btadapter.BIN.length; i++) {
        //
        //   if (SingletonFunctionController.btadapter.BIN[i]!.isNotEmpty) {
      ...
      paintUser(nearestBeacon, null, null, speakTTS: speakTTS,
          providePinSelection: pinSelectionMarker);
      ...
      for (int i = 0; i <
          SingletonFunctionController.btadapter.BIN.length; i++) {
        if (SingletonFunctionController.btadapter.BIN[i]!.isNotEmpty) {
          SingletonFunctionController.btadapter.BIN[i]!.forEach((key, value) {
            key = "";
            value = 0.0;
    ...
    String findMaxWeightKey(Map<String, List<int>> data) {
      ...
      for (var entry in data.entries) {
        ...
        double totalWeight = values.map((v) => getWeight(v.abs())).reduce((a,
            b) => a + b);
    ...
    void callbackFunc() {
      print("callback");
      SingletonFunctionController().executeFunction(
          buildingAllApi.allBuildingID).then((_) {
    ...
    Future<void> realTimeReLocalizeUser(
          ...
          if (SingletonFunctionController.apibeaconmap[key]!.coordinateX! !=
            ...
            position = tools.localtoglobal(
                ...
                SingletonFunctionController.building
                    .patchData[SingletonFunctionController.apibeaconmap[key]!
                    .sId! ??
      ...
      if (sumMap.isNotEmpty) {
        ...
        final Uint8List iconMarker = await getImagesFromMarker(
            'assets/EM_CurrentLocationMarker.png', 85);
        if (lastBeaconValue != firstValue &&
            sortedsumMap.entries.first.value >= 0.4) {
          SingletonFunctionController.btadapter.stopScanning();
          _exploreModeMarker.clear();
          await SingletonFunctionController.building.landmarkdata!.then((
              value) {
            ...
            getallnearestInfo.forEach((landmark) {
              ...
              if (landmark.coordinateX != null) {
                value = tools.localtoglobal(
                    ...
                    SingletonFunctionController.building.patchData[landmark
    ...
    void relocalizeUser() {
      ...
      if (!user.isnavigating && !isLocalized) {
        SingletonFunctionController.btadapter.stopScanning();
        if (Platform.isAndroid) {
          bluetoothScanAndroidClass.listenToScanInitialLocalization(
              Building.apibeaconmap).then((value) {
            setState(() {
              isLocalized = false;
            });
      ...
      if (value.patchData!.coordinates!.isNotEmpty) {
        ...
        setState(() {
          otherpatch.add(
            Polygon(
                polygonId: PolygonId(
                    'otherpatch ${value.patchData!.buildingID}'),
                points: polygonPoints,
                strokeWidth: 1,
                strokeColor: Color(0xffC0C0C0),
                fillColor: Color(0xffffffff),
    ...
    Future<BitmapDescriptor> bitmapDescriptorFromTextAndImageForPatchTransition(
      ...
      final ui.FrameInfo markerImageFrame = await markerImageCodec
          .getNextFrame();
    ...
    renderCampusPatchTransition(List<String> IDS, {String? outdoorID}) {
      ...
      if (blurPatch.length != IDS.length - (outdoorID == null ? 0 : 1)) {
        ...
        SingletonFunctionController.building.ARCoordinates.forEach((key,
            innerMap) {
          if (IDS.contains(key) && key != outdoorID) {
            currentCoordinated = innerMap;
            if (currentCoordinated.isNotEmpty) {
              List<LatLng> points = [];
              List<MapEntry<int, LatLng>> entryList = currentCoordinated.entries
                  .toList();
              entryList.sort((a, b) => a.key.compareTo(b.key));
              LinkedHashMap<int, LatLng> sortedCoordinates = LinkedHashMap
                  .fromEntries(entryList);
    ...
    void createotherARPatch(Map<int, LatLng> coordinates, String bid) async {
      ...
      if (coordinates.isNotEmpty) {
        ...
        if (SingletonFunctionController.building.ARCoordinates.containsKey(
            bid)) {
    ...
    Future<void> addselectedRoomMarker(List<LatLng> polygonPoints,
        String assetPath, {Color? color}) async {
      ...
      void updateMarkerSize() async {
        ...
        setState(() {
          if (selectedroomMarker.containsKey(
              buildingAllApi.getStoredString())) {
      ...
      try {
        ...
        for (int i = 0; i <= steps; i++) {
          ...
          final interpolatedLat = _lerp(
              center.latitude, bounds.northeast.latitude, t);
          final interpolatedLng = _lerp(
              center.longitude, bounds.northeast.longitude, t);
    ...
    LatLngBounds _adjustBoundsForPanels(LatLngBounds bounds,
        double paddingFactor) {
      final double latDiff = bounds.northeast.latitude -
          bounds.southwest.latitude;
      final double lngDiff = bounds.northeast.longitude -
...
// Linear interpolation helper
    ...
    Future<void> polygonTap(List<LatLng>? coordinates, String id) async {
      ...
      land? singletonData = await SingletonFunctionController.building
          .landmarkdata;
      Landmarks? landmark;
            ...
            if (polyArray.visibilityType == "visible" &&
              ...
              for (Nodes node in polyArray.nodes!) {
                ...
                coordinates.add(LatLng(
                    ...
                    tools.localtoglobal(
                        ...
                        node.coordy!,
                        SingletonFunctionController
                            .building.patchData[value.polyline!
                            .buildingID])[1]));
              }
              ...
              if (polyArray.polygonType == 'Wall' ||
                ...
                if (coordinates.length >= 2) {
                  polylines[value.polyline!.buildingID!]!.add(gmap.Polyline(
                      polylineId: PolylineId(
                          "${value.polyline!.buildingID!} Line ${polyArray
                              .id!}"),
                      points: coordinates,
                        ...
                        onTap: () {
                          ...
                          setState(() {
                            if (SingletonFunctionController.building
                              ...
                              PathState.directions = [];
                              interBuildingPath.clear();
                              addselectedRoomMarker(
                                  coordinates, 'assets/ATM.png');
                            }
                          });
                        }));
                  }
                  ...
                  if (coordinates.length > 2) {
                    ...
                    closedpolygons[value.polyline!.buildingID!]!.add(Polygon(
                        ...
                        onTap: () {
                          ...
                          moveCameraSmoothly(controller: _googleMapController,
                              targetPosition: CameraPosition(
                                  target: tools.calculateRoomCenterinLatLng(
                                      coordinates), zoom: 22),
                          ...
                          setState(() {
                            if (SingletonFunctionController.building
                              ...
                              interBuildingPath.clear();
                              addselectedRoomMarker(
                                  coordinates, 'assets/Generic Marker.png');
                            }
                          });
                        }));
                  }
                ...
                if (polyArray.cubicleName == "Green Area" ||
                    ...
                    (polyArray.name ?? "").toLowerCase().contains(
                        'auditorium') ||
                    (polyArray.name ?? "").toLowerCase().contains(
                        'basketball') ||
                  ...
                  if (coordinates.length > 2) {
                    ...
                    closedpolygons[value.polyline!.buildingID!]!.add(Polygon(
                        polygonId: PolygonId(
                            "${value.polyline!.buildingID!} Cubicle ${polyArray
                                .id!}"),
                        points: coordinates,
                        strokeWidth: 1,
                        // Modify the color and opacity based on the selectedRoomId
    ...
    Future<BitmapDescriptor> bitmapDescriptorFromTextAndImage(String text,
        String? imagePath,
      ...
      double canvasWidth = textWidth > imageSize.width ? textWidth : imageSize
          .width;
      double canvasHeight = textHeight + (imagePath != null
      ...
      if (imagePath != null) {
        ...
        // Draw the base marker image below the text
        final double imageX = (canvasWidth - imageSize.width) / 2;
        final double imageY = textHeight +
            10.0; // Padding between text and image
        canvas.drawImage(markerImage, Offset(imageX, imageY), Paint());
      }
    ...
    Timer? _timerCompass;
    Widget landmarkdetailpannel(BuildContext context,
        AsyncSnapshot<land> snapshot) {
      pathMarkers.clear();
      // if(user.isnavigating==false){
      //   clearPathVariables();
      // }
      ...
      if (!snapshot.hasData ||
          ...
          SingletonFunctionController.building.selectedLandmarkID] ==
              null) {
        print("landmark pannel got id ${SingletonFunctionController.building
            .selectedLandmarkID}");
        //
                      ...
                      child: Column(
                        ...
                        children: [
                                      ...
                                      children: [
                                        Text(
                                          "${snapshot.data!
                                              ...
                                              .building.selectedLandmarkID]!
                                              .properties!
                                              .startTime} AM- ${snapshot.data!
                                              .landmarksMap![SingletonFunctionController
                                              .building.selectedLandmarkID]!
                                              .properties!.endTime} PM  ",
                                          style: const TextStyle(
                                            fontFamily: "Roboto",
                                            fontSize: 16,
                                            fontWeight: FontWeight.w400,
                                    ...
                                    child: IconButton(
                                        onPressed: () {
                                          HelperClass.shareContent(
                                              "${AppConfig
                                                  .baseUrl}/#/landmarkId/${SingletonFunctionController
                                                  .building
                                                  .selectedLandmarkID!}",
                                              snapshot.data!
                                                  .landmarksMap![SingletonFunctionController
                          ...
                          snapshot
                              ...
                              ? InkWell(
                            onTap: () {
                              HelperClass.makePhoneCall(snapshot.data!
                                  .landmarksMap![SingletonFunctionController
                                  .building.selectedLandmarkID]!.properties!
                                  .contactNo!);
                            },
                            child: Container(
                          ...
                          snapshot
                              ...
                              ? InkWell(
                            onTap: () {
                              HelperClass.sendMailto(email: snapshot.data!
                                  .landmarksMap![SingletonFunctionController
                                  .building.selectedLandmarkID]!.properties!
                                  .email!);
                            }, child: Container(
                          ...
                          snapshot
                              ...
                              ? InkWell(
                            onTap: () {
                              HelperClass.launchURL(snapshot.data!
                                  .landmarksMap![SingletonFunctionController
                                  .building.selectedLandmarkID]!.properties!
                                  .url!);
                            },
                        ...
                        onPressed: () async {
                          if (tappedPolygonCoordinates.isNotEmpty) {
                            moveCameraSmoothly(controller: _googleMapController,
                                targetPosition: CameraPosition(
                                    ...
                                    zoom: 22),
                                currTarget: tools.calculateRoomCenterinLatLng(
                                    tappedPolygonCoordinates));
                          }
                          _polygon.clear();
                          cachedPolygon.clear();
                          // circles.clear();
                          ...
                          if (user.coordY != 0 && user.coordX != 0) {
                            ...
                            PathState.destinationFloor = snapshot
                                ...
                                .building.selectedLandmarkID]!
                                .floor!;
                            print("userbid in landmarkdetailpannel is ${user
                                .bid}");
                            PathState.sourceBid = user.bid;
    ...
    List<CommonConnection> findCommonLifts(Landmarks landmark1,
        Landmarks landmark2, String accessibleby) {
    ...
    Future<void> fitTwoPoints(List<LatLng> points) async {
      ...
      LatLngBounds bounds = LatLngBounds(
        southwest: LatLng(
          ...
          point1.longitude < point2.longitude ? point1.longitude : point2
              .longitude,
        ),
        northeast: LatLng(
          point1.latitude > point2.latitude ? point1.latitude : point2.latitude,
    ...
    Future<void> runPaths(List<dynamic Function()> fetchrouteFutures,
      ...
      if (PathState.sourceBid != buildingAllApi.outdoorID &&
        ...
        if (PathState.sourceBid == PathState.destinationBid) {
          ...
          SingletonFunctionController.building.floor[PathState.sourceBid] =
              PathState.sourceFloor;
          SingletonFunctionController.building.floor[PathState.destinationBid] =
          0;
          createRooms(
              SingletonFunctionController
                  .building.polylinedatamap[PathState.sourceBid]!,
      ...
      if (fetchrouteFutures.isNotEmpty) {
        ...
        for (int i = 0; i < result.length - 1; i++) {
          ...
          singleroute["${buildingAllApi.outdoorID}"]!.putIfAbsent(
              0, () => Set());
        ...
        for (var res in result) {
          ...
          lifts.add(data["lift"]);
          print(
              "PathState.singleCellListPath ${PathState.singleCellListPath
                  .length}");
    ...
    Future<void> calculateroute(Map<String, Landmarks> landmarksMap,
      ...
      if (accessibleby == null) {
        if (landmarksMap[PathState.sourcePolyID]!.escalators != null &&
            landmarksMap[PathState.sourcePolyID]!.escalators!.any((
                escalator) => (escalator.distance ?? 100) <= 45)) {
      ...
      print("calculateroutesource ${PathState.sourceX},${PathState
          .sourceY} <> ${PathState.sourceBid},${PathState.sourceFloor}");
      print("calculateroutedestination ${PathState.destinationX},${PathState
          .destinationY} <> ${PathState.destinationBid},${PathState
          .destinationFloor}");
      if (PathState.sourceBid == PathState.destinationBid) {
        if (PathState.sourceFloor == PathState.destinationFloor) {
          ...
          List<double> dvalue = [];
          svalue = tools.localtoglobal(
              PathState.sourceX, PathState.sourceY,
              SingletonFunctionController.building.patchData[PathState
                  .sourceBid]);
          dvalue = tools.localtoglobal(
        ...
        SingletonFunctionController.building.landmarkdata!.then((land) async {
          ...
          /// sourceEntryINCAMPUS
          Landmarks? CampusSourceEntry = await findCampusEntry(
              land, sourceEntry);

          print(
              "CampusSourceEntry is ${CampusSourceEntry
                  ?.name} [${CampusSourceEntry?.coordinateX},${CampusSourceEntry
                  ?.coordinateY}]");
          print(
              "CampusDestinationEntry is ${CampusDestinationEntry
                  ?.name} [${CampusDestinationEntry
                  ?.coordinateX},${CampusDestinationEntry?.coordinateY}]");
          ...
          if (PathState.sourceFloor == sourceEntry.floor) {
            ...
            List<dynamic> commonlifts = findCommonLifts(
                landmarksMap[PathState.sourcePolyID]!, sourceEntry,
                accessibleby ?? "Lifts");
            if (commonlifts.isEmpty) {
              setState(() {
                PathState.noPathFound = true;
    ...
    void CampusPathAPIAlgorithm(Landmarks sourceEntry,
        Landmarks destinationEntry) async {
      ...
      destinationEntrylat =
          double.parse(destinationEntry.properties!.latitude!);
      destinationEntrylng =
          double.parse(destinationEntry.properties!.longitude!);
      ...
      List<double> sourceEntryCoordinates = tools.localtoglobal(
          sourceEntry.coordinateX!,
          sourceEntry.coordinateY!,
          SingletonFunctionController.building.patchData[sourceEntry
              .buildingID]);
    ...
    String currentBid = "";
    int currentCols = 0;
    Future<Map<String, dynamic>> fetchroute(int sourceX, int sourceY,
        int destinationX, int destinationY, int floor,
        {String? bid = null,
          int? nextFloor,
          String? liftName,
      ...
      print(
          "checks for campus $bid ${SingletonFunctionController.building
              .floorDimenssion[bid]}");
      print(
      ...
      int destinationIndex = calculateindex(
          destinationX, destinationY, numCols);
      PathState.numCols ??= {};
      ...
      try {
        ...
        PathModel model = Building.waypoint[bid]!.firstWhere((
      ...
      if (path.isEmpty) {
        ...
        networkManager.ws.updatePath(didPathForm: path[0] == sourceIndex &&
            path[path.length - 1] == destinationIndex);
      }

      if (bid == buildingAllApi.outdoorID) {
        path.forEach((turn) =>
            getPoints.add([turn % numCols, turn ~/ numCols]));
      } else {
          ...
          try {
            innerMarker.add(Marker(
              ...
              icon: await CustomMarker(
                  ...
                  Icons.elevator_outlined : (PathState.accessiblePath ==
                      'Stairs') ? Icons.escalator : (PathState.accessiblePath ==
                      'Ramps') ? Icons.accessible : Icons.escalator_warning)
                  .toBitmapDescriptor(
              ...
              onTap: () {
                if (!user.isnavigating) {
                  ...
                  SingletonFunctionController.building.landmarkdata!.then((
                      value) {
                    createMarkers(
                        value,
                        SingletonFunctionController
        ...
        tools
          ...
          if (liftName != null) {
            directions.add(direction(
                -1,
                "Take ${liftName} and Go to ${PathState
          ...
          directions.addAll(
              tools.getDirections(path, value, PathState, lifts, context));
    ...
    Future<void> callPreviewAnimation() async {
      ...
      await SingletonFunctionController.building.landmarkdata!.then((
        ...
        List<Landmarks> nearbyLandmarks = tools.findNearbyLandmark(
            user.cellPath, value.landmarksMap!, 5);
        ...
        for (int i = 0; i < pathList.length - 2; i++) {
          ...
          if (singleroute[currentBid]![currentFloor] != null) {
            gmap.Polyline oldPolyline = singleroute[currentBid]![currentFloor]!
                .firstWhere(
              ...
              if (i < nearbyLandmarks.length - 1) {
                try {
                  List<double> value1 = tools.localtoglobal(
                      ...
                      SingletonFunctionController.building
                          .patchData[currentBid]);
                  innerMarker.add(
                    Marker(
                      markerId: MarkerId('previewMarker${nearbyLandmarks[i]
                          .sId}'),
          ...
          if (turnPoints.contains(node)) {
            ...
            await alignMapToPath(
                [value[0], value[1]], [value1[0], value1[1]], isTurn: true);
          } else {
                      ...
                      child: Container(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              AccessiblePathButton(label: "Stairs",
                                  icon: Icons.escalator,
                                  accessibleBy: "Stairs",
                                  PathState: PathState,
                                  calculateroute: calculateroute),
                              AccessiblePathButton(label: "Lift",
                                  icon: Icons.elevator,
                                  accessibleBy: "Lifts",
                                  PathState: PathState,
                                  calculateroute: calculateroute),
                              AccessiblePathButton(label: "Escalator",
                                  icon: Icons.escalator_warning,
                                  accessibleBy: "Escalators",
                                  PathState: PathState,
                                  calculateroute: calculateroute),
                              AccessiblePathButton(label: "Ramp",
                                            ...
                                            Semantics(
                                              ...
                                              child: IconButton(
                                                  ...
                                                  icon: SvgPicture.asset(
                                                    'assets/routeDetailPannel_ShareIcon.svg',
                                                    color: Colors.black,)),
                                            ),
                                            ...
                                            children: [
                                              Semantics(
                                                ...
                                                child: DirectionInstruction(
                                                  viewModel: DirectionInstructionViewModel(
                                                      PathState.directions,
                                                      PathState.sourceBid,
                                                      PathState.sourceName,
                                                      PathState.sourceFloor,
      ...
      return Visibility(
        ...
        child: Focus(
          ...
          child: Semantics(
            ...
            label: "You’ve Arrived ${destiN.isEmpty
                ? "Your Destination"
                : destiN}. It is ${finalDestinationDirection}",
            child: Container(
              child: Column(
    ...
    Future<void> alignMapToPath(List<double> A, List<double> B,
        {bool isTurn = false}) async {
      print("enteredddd");
      print(onStart);
      double screenHeight = MediaQuery
          .of(context)
      ...
      ScreenCoordinate screenCenter = await _googleMapController
          .getScreenCoordinate(mapState.target);
      ...
      LatLng newCameraTarget = await _googleMapController.getLatLng(
          ScreenCoordinate(x: newX, y: newY));
      setState(() {
        if (isTurn) {
          Future.delayed(Duration(microseconds: 2500)).then((onValue) {
            _googleMapController.animateCamera(CameraUpdate.newCameraPosition(
              CameraPosition(
                  target: (onStart == false) ? mapState.target : mapState
                      .target,
                  zoom: mapState.zoom,
                  bearing: mapState.bearing!,
                  tilt: mapState.tilt),
...
// Function to resume the compass subscription
    ...
    void getPathDirections() {
      setState(() {
        user.theta = tools.calculateBearing_fromLatLng(
            LatLng(user.lat, user.lng), LatLng(
        ...
        if (mapState.interaction2) {
          ...
          if (markers.length > 0 && markers[user.bid] != null)
            markers[user.bid]![0] = customMarker.rotate(
                tools.calculateBearing_fromLatLng(LatLng(user.lat, user.lng),
                    LatLng(user.cellPath[user.pathobj.index + 1].lat,
                        user.cellPath[user.pathobj.index + 1].lng)) -
                    mapbearing, markers[user.bid]![0]);
        }
      });
    }
    ...
    Widget navigationPannel() {
      ...
      DateTime currentTime = DateTime.now();
      if (PathState.singleCellListPath.isNotEmpty) {
        distance = tools.PathDistance(
            PathState.singleCellListPath, index: user.pathobj.index);
        time = distance / 120;
        time = time.ceil().toDouble();
      ...
      try {
        ...
        if (user.isnavigating && user.pathobj.numCols![user.bid] != null) {
          ...
          if (MotionModel.reached(user, col) == false &&
            ...
            //
            List<int> b = [
              user.showcoordX + tval[0],
              user.showcoordY + tval[1]
            ];
...
// Call filterItems() whenever tags change
    ...
    Widget nearestLandmarkpannel() {
      ...
      return Visibility(
          ...
          child: Semantics(
            label: "You are near ${user.locationName}, ${LocaleData.floor
                .getString(context)} ${user.floor}",
            excludeSemantics: true,
            child: SlidingUpPanel(
              controller: _panelController,
              borderRadius: BorderRadius.all(Radius.circular(24.0)),
    ...
    Set<Marker> getCombinedMarkers() {
      ...
      buildingAllApi.allBuildingID.forEach((key, value) {
        if (pathMarkers[key] != null &&
            pathMarkers[key]![SingletonFunctionController.building
                .floor[key]] !=
                null) {
        ...
        if ((!_isRoutePanelOpen || !_isnavigationPannelOpen) &&
          ...
          combinedMarkers =
              combinedMarkers.union(Set<Marker>.of(markers[key]!));
        }
      });
    ...
    Set<gmap.Polyline> getCombinedPolylines() {
      ...
      buildingAllApi.allBuildingID.forEach((key, value) {
        if (singleroute[key] != null &&
            singleroute[key]![SingletonFunctionController.building
                .floor[key]] !=
                null) {
      ...
      buildingAllApi.allBuildingID.forEach((key, value) {
        if (pathCovered[key] != null &&
            pathCovered[key]![SingletonFunctionController.building
                .floor[key]] !=
                null) {
      ...
      if (SingletonFunctionController.building.updateMarkers) {
        ...
        if (user.isnavigating) {
          ...
          setState(() {
            Markers.forEach((marker) {
              List<String> words = marker.markerId.value.split(' ');
              if (SingletonFunctionController.building.ignoredMarker.contains(
                  words[1])) {
                if (marker.markerId.value.contains("Door")) {
    ...
    void _updateBuilding(double zoom) {
      ...
      setState(() {
        ...
        polylines[buildingAllApi.getStoredString()]!.forEach((polyline) {
          gmap.Polyline _polyline = polyline.copyWith(
              visibleParam: zoom > 16.0);
    ...
    void onLandmarkVenueClicked(String ID,
      ...
      if (snapshot.landmarksMap![ID]!.buildingID != buildingAllApi.outdoorID &&
          SingletonFunctionController.building.floor[snapshot.landmarksMap![ID]!
              .buildingID] != floor) {
        SingletonFunctionController.building.floor[snapshot.landmarksMap![ID]!
            .buildingID!] = floor;
        createRooms(
            SingletonFunctionController.building.polylinedatamap[snapshot
                .landmarksMap![ID]!.buildingID]!, floor);
    ...
    void fromSourceAndDestinationPage(List<String> value) {
      ...
      SingletonFunctionController.building.landmarkdata!.then((land) {
        ...
        tools.setBuildingAngle(SingletonFunctionController
            .building.patchData[PathState.sourceBid]!.patchData!
            .buildingAngle!);
    ...
    Widget build(BuildContext context) {
      ...
      double statusBarHeight = MediaQuery
          ...
          .padding
          .top;
      isSemanticEnabled = MediaQuery
          .of(context)
          .accessibleNavigation;
      HelperClass.SemanticEnabled = MediaQuery
          .of(context)
          .accessibleNavigation;
      ...
      Scaffold(
        body: Stack(
          children: [
            ...
            Semantics(
              ...
              child: Container(
                child: GoogleMap(
                  ...
                  polygons: getCombinedPolygons().union(_polygon).union(
                      globalCampus),
                  ...
                  markers: getCombinedMarkers()
                      ...
                      .union(Markers)
                      .union(restBuildingMarker).union(debugMarker).union(
                      GpsMarker).union(nearbyLandmarks.values.toSet()).union(
                      _exploreModeMarker)
                  ...
                  onCameraMove: (CameraPosition cameraPosition) {
                    mapState.cameraposition =
                        cameraPosition; // User has started panning
                    ...
                    mapState.aligned =
                        cameraPosition.target.latitude.toStringAsFixed(5) ==
            ...
            Positioned(
              bottom: isLiveLocalizing ? screenHeight * 0.6 : 150.0,
              ...
              child: Semantics(
                ...
                child: Column(
                  children: [
                    ...
                    !isLiveLocalizing ? isSemanticEnabled &&
                        _isRoutePanelOpen ||
                        isSemanticEnabled && _isLandmarkPanelOpen ||
                        PinLandmarkPannel.isPanelOpened()
                        ? Container()
                        : Semantics(
                        ...
                        if (user.initialallyLocalised) {
                          ...
                          setState(() {
                            if (isLiveLocalizing) {
                              ...
                              speak(
                                  "${LocaleData.exploremodenabled.getString(
                                      context)}", _currentLocale);
                              exploremodeLandmarkTimer = Timer.periodic(
                        ...
                        child: AnimatedBuilder(
                          ...
                          builder: (context, child) {
                            return LinearProgressIndicator(
                              ...
                              // Apply the border radius directly
                              value: PBanimation.value,
            ...
            detected ? Semantics(
                child: SafeArea(child: nearestLandmarkpannel())) : Container(),
            SizedBox(height: 28.0), // Adjust the height as needed
            (SingletonFunctionController.building.buildingsLoaded ||
            ...
            ExcludeSemantics(child: Visibility(
